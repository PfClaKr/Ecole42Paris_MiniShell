...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/16 18:17:54 by ychun             #+#    #+#             */
/*   Updated: 2023/01/16 18:23:44 by ychun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_final_line(char **final_line)
{
	...
	new_final_line = malloc(sizeof(char)
			* (ft_strlen_gnl(*final_line) - i + 1));
	if (!new_final_line)
		return (0);
	while ((*final_line)[++i])
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (0);
	while (!ft_strchr_gnl(final_line, '\n') && len_read > 0)
	{
		len_read = read(fd, buffer, BUFFER_SIZE);
		buffer[len_read] = '\0';
		final_line = ft_strjoin_gnl(final_line, buffer);
	}
	free(buffer);
